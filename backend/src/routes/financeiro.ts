 ...
 */
router.get('/logs', authenticateToken, requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  const { tenant } = req;
 ...
 * Limpa todos os logs
 */
router.delete('/logs', authenticateToken, requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  const { tenant, user } = req;
