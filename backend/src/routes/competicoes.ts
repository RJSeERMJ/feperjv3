...
import { Competicao, ApiResponse, PaginationParams, PaginatedResponse } from '../types';
...
router.get('/', authenticateToken, asyncHandler(async (req: Request, res: Response) => {
  const { tenant } = req;
  const { page = 1, limit = 50, search, sortBy = 'nomeCompeticao', sortOrder = 'desc' } = req.query as PaginationParams;
  ...
  if (search) {
    ...
    filteredCompeticoes = competicoes.filter(competicao => 
      competicao.nomeCompeticao.toLowerCase().includes(searchLower) ||
  ...
  filteredCompeticoes.sort((a, b) => {
    const aValue = a[sortBy as keyof Competicao];
    const bValue = b[sortBy as keyof Competicao];
  ...
  const response: PaginatedResponse<Competicao> = {
...
router.post('/', authenticateToken, requireAdminOrChefe, asyncHandler(async (req: Request, res: Response) => {
  ...
  const competicaoData: Omit<Competicao, 'id'> = req.body;
...
router.put('/:id', authenticateToken, requireAdminOrChefe, asyncHandler(async (req: Request, res: Response) => {
  const { tenant, user } = req;
  const { id } = req.params;
  const competicaoData: Partial<Competicao> = req.body;
...
export { router as competicoesRoutes };
